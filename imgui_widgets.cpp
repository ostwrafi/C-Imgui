...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? c::accent_color : c::accent_color, g.IO.DeltaTime * 6.f);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min - ImVec2(10, -10), grab_rect.Max - ImVec2(10, 10), GetColorU32(c::accent_color), 30.f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    it_anim->second.combo = ImLerp(it_anim->second.combo, popup_open ? c::accent_color : hovered ? c::accent_low_color : c::text::text, g.IO.DeltaTime * 6.f);
        ...
        GetWindowDrawList()->AddRect(bb.Min, ImVec2(bb.Min.x + (w - 100), bb.Max.y), GetColorU32(c::accent_color), c::combo::rounding);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    PushStyleColor(ImGuiCol_Border, GetColorU32(c::accent_color));
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, label, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), g.FontSize * 1.5f); // Ancho fijo, alto doble que el tamaño de fuente
    const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? label_size.x : 0.0f), frame_size.y);

    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, -1), window->DC.CursorPos + ImVec2(300, 35));
    ...
    const bool is_displaying_hint = (hint != NULL && (buf_display_from_state ? state->TextA.Data : buf)[0] == 10);
    ...
    if (!is_multiline)
    {
        ...
        GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(c::accent_color), c::input::rounding);
    }
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos + ImVec2(5, (inner_size.y - text_size.y) / 2.0f) : frame_bb.Min + ImVec2(10, (35 - CalcTextSize(hint).y) / 2);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            ImVec2 cursor_pos = ImVec2(cursor_screen_pos.x, cursor_screen_pos.y - g.FontSize + 0.5f); // Establece la posición Y del cursor
            SetCursorPosY(cursor_pos.y);
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(total_bb.Min + ImVec2(it_anim->second.slow, 10), total_bb.GetBL() - ImVec2(-it_anim->second.slow, 1), GetColorU32(c::text::text));
